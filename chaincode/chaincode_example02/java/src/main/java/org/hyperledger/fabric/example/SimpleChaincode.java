...
public class SimpleChaincode extends ChaincodeBase {
	...
	public Response init(ChaincodeStub stub) {
		try {
			...
			_logger.info(String.format("account %s, value = %s; account %s, value %s", account1Key, account1Value,
	...
	public Response invoke(ChaincodeStub stub) {
		try {
			...
			return newErrorResponse(
					"Invalid invoke function name. Expecting one of: [\"invoke\", \"delete\", \"query\",\"fastDFsUploadFile\",\"fastDFsDownloadFile\"]");
		} catch (Throwable e) {
			return newErrorResponse(e);
		}
	}
	 ...
	 * TrackerServer trackerServer =null; StorageServer storageServer = null;
	 * 
	 * String[] downfilepath = path.split("/" , 2); String groupId =
	 * downfilepath[0]; String filepath = downfilepath[1]; try {
	 * ClientGlobal.init(conf_filename); TrackerClient tracker = new
	 * TrackerClient(); trackerServer = tracker.getConnection();
	 * 
	 ...
	 * byte[] bytes = storageClient.download_file(groupId, filepath);
	 * 
	 * OutputStream out = new FileOutputStream(storePath); out.write(bytes);
	 * out.close(); } catch (Exception e) { e.printStackTrace(); } finally { try {
	 * if(null!=storageServer) storageServer.close(); if(null!=trackerServer)
	 * trackerServer.close(); } catch (IOException e) { // TODO Auto-generated catch
